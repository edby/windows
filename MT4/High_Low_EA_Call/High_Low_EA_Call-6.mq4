...
extern int Move_Stop_Small=7,Move_Stop_Big=40;  //Move_Stop_Loss()
int Sleep_Time,Stop_Loss=5;
...
int start()
{
  static int x_1=0;
  if(x_1==0)
  {Print("Move_Stop:",Move_Stop_Small,Move_Stop_Big);x_1=1;}
...
*/
  ...
  double Trade_Sig=iCustom(NULL,0,"Test_Ind",15,9,7,2,0);
  int Shift_Bar=iCustom(NULL,0,"Test_Ind",15,9,7,2,1);
  ...
  {  
    ...
    else if(OrderSelect(0,SELECT_BY_POS,MODE_TRADES)&&OrderType()==OP_BUY&&(TimeCurrent()-OrderOpenTime())/(Time[1]-Time[2])>10) // add the last condition
    {
      iWait(); 
   ...
   if(MathAbs(Trade_Sig-1.0)<Point)
  {
    ...
    else if(OrderSelect(0,SELECT_BY_POS,MODE_TRADES)&&OrderType()==OP_SELL&&(TimeCurrent()-OrderOpenTime())/(Time[1]-Time[2])>10)// add the last condition
    {
      iWait(); 
...
{                                 // bigger than small stop level, and when order profit is more than 20 points, the stop level should 
  ...
  for(int i=OrdersTotal();i>0;i--)
  {
    if(OrderSelect(i-1,SELECT_BY_POS))
    {
      ...
      int Move_Stop=Move_Stop_Small;
      if(Profit_Point>Move_Stop_Small*Point_Adapted/Point&&Profit_Point<=400) Move_Stop=Move_Stop_Big; 
      if(Profit_Point>400)Move_Stop=Move_Stop_Small;
      if(MathAbs(Close[0]-OrderStopLoss())>Move_Stop*Point_Adapted)
      if(OrderType()==OP_BUY) {iWait();OrderModify(OrderTicket(),OrderOpenPrice(),Close[0]-Move_Stop*Point_Adapted,0,0,0);}
      else if(OrderType()==OP_SELL) {iWait();OrderModify(OrderTicket(),OrderOpenPrice(),Close[0]+Move_Stop*Point_Adapted,0,0,0);}
    } 
}
  return;
}
