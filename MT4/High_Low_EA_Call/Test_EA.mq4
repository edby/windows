...
extern int Move_Stop_Small=20,Move_Stop_Big=40;  //Move_Stop_Loss()
int Sleep_Time,Stop_Loss=7;
...
*/
  ...
  double Trade_Sig=iCustom(NULL,0,"Test_Ind",15,9,3,2,0);
  int Shift_Bar=iCustom(NULL,0,"Test_Ind",15,9,3,2,1);
  ...
  {  
    ...
    else if(OrderSelect(0,SELECT_BY_POS,MODE_TRADES)&&OrderType()==OP_BUY) // &&(TimeCurrent()-OrderOpenTime())/(Time[1]-Time[2])>6) 
    {
      iWait(); 
   ...
   if(MathAbs(Trade_Sig-1.0)<Point)
  {
    ...
    else if(OrderSelect(0,SELECT_BY_POS,MODE_TRADES)&&OrderType()==OP_SELL)// &&(TimeCurrent()-OrderOpenTime())/(Time[1]-Time[2])>6)
    {
      iWait(); 
...
{                                 // bigger than small stop level, and when order profit is more than 20 points, the stop level should 
  ...
  for(int i=OrdersTotal();i>0;i--)
  {
    ...
    {
      int Profit_Point=NormalizeDouble(MathAbs(Close[0]-OrderOpenPrice())/Point_Adapted,0); 
      int Move_Stop=Move_Stop_Small;
      if(Profit_Point>20&&Profit_Point<=50) Move_Stop=Move_Stop_Big; //
      if(Profit_Point>50) Move_Stop=Move_Stop_Small;
