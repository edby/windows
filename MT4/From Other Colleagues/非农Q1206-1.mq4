...
extern int 止盈点数 = 600;
double TP;
extern int huadian = 10;
extern int 订单特征码 = 2013120600;
int MyMagicNumFN;
//yidongzhisun
extern int MinProfitPoint = 500;
extern double 移动止损间隔点数 = 200;
double TrallingStop;

extern string 开仓时间 = "15:30:00";   
string MyTimeStart;
...
//-----------------------------------------------------------------------------------------
      ...
      //    Print("此时的仓位保证金不够,值为:",MyFreeMarginCheck);
          ...
          {
            Print("此时的仓位保证金不够,值为:",MyFreeMarginCheck);
            int sut = MessageBox("此时的保证金不够,值为"+DoubleToStr(MyFreeMarginCheck,Digits)+"!\n","AccountFreeMarginCheck",MB_OK);
            Print("111 "+sut);
   ...
   {      
      ...
      {           
         ...
         {          
            ...
            {             
         //----------------------------------------------------------------------------------------------------------------------------         
              if(Bid - OrderOpenPrice() > MinProfitPoint * MyPoint)
              {                    
           ...
           {
             if(OrderOpenPrice() - Ask > MinProfitPoint * MyPoint)
             {
