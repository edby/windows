...
#property  indicator_color1  Red
  ...
  int init()
  { 
    Print("Seek_Period, Bar_Ign, Range_Ratio, Delta_Ratio: ",Seek_Period,",",Bar_Ign,",",Range_Ratio,",",Delta_Ratio);
    IndicatorBuffers      (2)                             ;
  ...
  int start()
  {
   ...
   Signal[0]  = 0.0 ;
   if( Close[0]-PT_Buffer[0] >= Delta_Ratio*Pips_SL ) //buy  
   { 
     int  CNT   =  1     ;
     bool Check =  false ; 
     while( CNT <= Bar_Ign )
    ...
    if( PT_Buffer[0]-Close[0] >= Delta_Ratio*Pips_SL ) //sell 
   { 
     CNT   = 1         ;
     Check = false     ;
     while( CNT <= Bar_Ign )
