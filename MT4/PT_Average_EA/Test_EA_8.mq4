...
#include <stdlib.mqh>
  ...
  int start()
  {
    ...
    if(Signal!=0 && OrderSelect(OrdersHistoryTotal()-1,SELECT_BY_POS,MODE_HISTORY))
      if(OrderProfit()>300 && 
         iBarShift(NULL,0,OrderOpenTime())-iBarShift(NULL,0,OrderCloseTime())>100 &&
...
/*   
   if( OrderSelect(OrdersHistoryTotal()-1,SELECT_BY_POS,MODE_HISTORY) && OrdersTotal()==0)
   {
    ...
    if(B_S<=x && OrderProfit()<0)
      if(OrderType()==OP_BUY && High[B_S]-Low[B_S] > 35*Pips_SL && Close[0]-Low[B_S]>30*Pips_SL)
      ...
      else if (OrderType()==OP_SELL && High[B_S]-Low[B_S] > 35*Pips_SL && High[B_S]-Close[0]>30*Pips_SL)
...
//    Move_Stop_Loss() ;
    ...
    if(OrderSelect(OrdersHistoryTotal()-1,SELECT_BY_POS,MODE_HISTORY)&&Signal!=0)
      if(OrderProfit()<0 && iBarShift(NULL,0,OrderCloseTime())<=3)
         if( (OrderType()==OP_BUY  && Signal == 1 ) ||
             (OrderType()==OP_SELL && Signal == -1)
           ) Signal =0;
  ...
  {        
    if( OrderSelect(OrdersTotal()-1,SELECT_BY_POS) )
    if(iBarShift(NULL,0,OrderOpenTime())>=2)
    { 
