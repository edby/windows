...
#include <stdlib.mqh>
  ...
  extern double
      Range_Ratio_Ind = 9
  ,   Slip            = 3
  ,   Move_Ratio      = 0.24   //0.21;0.24;0.43  these three values are better according to both risk and profit.
  ;
  ...
  { 
    ...
    else if( OrderSelect(OrdersTotal()-1,SELECT_BY_POS) )
    {
      ...
      { 
        ...
        else if( (TimeCurrent()-OrderOpenTime())/Bar_Seconds > 0 ) // Reset Stop Loss Line
        { 
          ...
          if( Stop_Loss > OrderStopLoss())
          {
            ...
            if( ((Gear ==  1 || Gear_A == 1) && OrderOpenPrice() < Stop_Loss && OrderOpenPrice() > 0.0)  || 
              ...
              { if     (OrdersTotal()==1) double L=0.4;
                else if(OrdersTotal()==2)        L=0.3;
                else                             L=0.2;
