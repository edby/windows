...
#include <stdlib.mqh>
  ...
  extern double
      Range_Ratio_Ind = 9
  ,   Slip            = 3
  ,   Move_Ratio      = 0.24   //0.21;0.24;0.43  these three values are better according to both rish and profit.////////////////////////
  ;
...
/* not matter much, can be deleted all///////////////////////////////////////////////////////
...
/* not matter much, can be deleted.//////////////////////////////////////////////////////////////////
...
*/
    ...
    // this one should be test exist or not.//////////////////////////////////////////////////////////////////////////////
    ...
    else if( OrderSelect(OrdersTotal()-1,SELECT_BY_POS) )
    {
      ...
      { 
        ...
        { 
          ...
          {
            if(   Gear == -3 || Gear_A == -3) OrderSelect(0,SELECT_BY_POS);
            if( ((Gear ==  1 || Gear_A == 1) && OrderOpenPrice() < Stop_Loss && OrderOpenPrice() > 0.0)  || 
                 (Gear ==  2 || Gear_A == 2)                                                             ||
      ...
      else if( OrderType() == OP_SELL) 
      {
        ...
        { 
          ...
          {
            if(   Gear == -3 || Gear_A == -3) OrderSelect(0,SELECT_BY_POS);
            if( ((Gear ==  1 || Gear_A == 1) && OrderOpenPrice() > Stop_Loss)        || 
                 (Gear ==  2 || Gear_A == 2)                                         ||
...
//单子加仓时候，加一个反向单对冲，该反向单设置止损止盈；亏损的单子后再反向开仓。
...
#include <stdlib.mqh>
  ...
  extern double
      Range_Ratio_Ind = 9
  ,   Slip            = 3
  ,   Move_Ratio      = 0.24   //0.21;0.24;0.43
  ;
  ...
  {        
    ...
    { 
      ...
      ;
      double   Profit_Points = (High[Bar_High] - Low[Bar_Low])/Pips_SL;
      if( Profit_Points > x && OrderType() == OP_BUY && OrderProfit()>0 && TimeCurrent()-Time[Bar_Low]!=0)
        if(Profit_Points*60/(TimeCurrent()-Time[Bar_Low]) > Move_Ratio && OrderClosePrice()-OrderStopLoss() > SL_Fast*Pips_SL)
          if( !OrderModify(OrderTicket(),OrderOpenPrice(),OrderClosePrice()-SL_Fast*Pips_SL,0,0,0) )
            Print("OrderModify() for Fast order failed: ",ErrorDescription(GetLastError()));
      if( Profit_Points > x && OrderType() == OP_SELL && OrderProfit()>0 && TimeCurrent()-Time[Bar_High]!=0)
        if(Profit_Points*60/(TimeCurrent()-Time[Bar_High]) > Move_Ratio && OrderStopLoss()-OrderClosePrice() > SL_Fast*Pips_SL)
          if( !OrderModify(OrderTicket(),OrderOpenPrice(),OrderClosePrice()+SL_Fast*Pips_SL,0,0,0) )
            Print("OrderModify() for Fast order failed: ",ErrorDescription(GetLastError()));    
    }
...
/*  
  void Order_Add()
  {
    if(OrderSelect(OrdersTotal()-1,SELECT_BY_POS)&&iBarShift(NULL,0,OrderOpenTime())>3)
    {
      if(OrderType()==OP_BUY)
        if(Low[3]==Low[iLowest(NULL,0,MODE_LOW,10,0)])
        { 
          double StopLoss = Set_Stop_Loss(1,OP_BUY);
          if(OrderSend(Symbol(),OP_BUY,Lots,Ask,Slip*Pips_Slip,StopLoss,0,0,Magic_Number_A,0,0)<0)
          Print(" Order Send failed: ",GetLastError());
        }        

      if(OrderType()==OP_SELL)
        if(High[3]==High[iHighest(NULL,0,MODE_HIGH,10,0)])
        { 
          StopLoss = Set_Stop_Loss(1,OP_SELL);
          if(OrderSend(Symbol(),OP_SELL,Lots,Bid,Slip*Pips_Slip,StopLoss,0,0,Magic_Number_A,0,0)<0)
          Print(" Order Send failed: ",GetLastError());
        }
    }
