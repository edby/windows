//+------------------------------------------------------------------------------------------------------------------+  
//| 1. Gear=2, Gear_A=-2,Gear_A_N=1,Lots=0.2,Move_Ratio=0.21; Net Profit:2.1 Max Drawdown:30%   Profit Factor:1.63   |  
//| 2. Gear=-3,Gear_A=0,            Lots=0.2,Move_Ratio=0.21; Net Profit:1.1 Max Drawdown:7.4%  Profit Factor:1.51   | 
...
#include <stdlib.mqh>
  ...
  { 
    ...
    Signal =  iCustom(NULL,0,"Test_Ind",Seek_Period_Ind,Bar_Ignore_Ind,Min_Points_Ind,Max_Points_Ind,Range_Ratio_Ind,1,0);
    ...
    else if( OrderSelect(Tickets[Orders_CNT-1],SELECT_BY_TICKET) )
    {
      ...
      { 
        ...
        { 
          ...
          {
            if(   Gear == -3 || Gear_A == -3) OrderSelect(Tickets[0],SELECT_BY_TICKET);
      ...
      else if( OrderType() == OP_SELL) 
      {
        ...
        { 
          ...
          if(OrderStopLoss()-Stop_Loss > Point && Stop_Loss > 0.0)
          {
            if(   Gear == -3 || Gear_A == -3) OrderSelect(Tickets[0],SELECT_BY_TICKET);
  ...
  void Ignore_Signal()  // ignore the continuous same signal within very few time
  {
    int T=-1;
