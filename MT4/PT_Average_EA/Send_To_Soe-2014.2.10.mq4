#include <stdlib.mqh>      // For Testing
  ...
  int start()
  { 
    if(TimeCurrent()>D'2014.3.30') Alert("Out of Date!");
    if(TimeCurrent()>D'2014.3.30' || 
       Period() != PERIOD_M15     || 
       Symbol() != "USDJPY"
      ) 
    return(0);
    int i=0,j=0;
    Signal =  iCustom(NULL,0,"X_Ind",Seek_Period_Ind,Bar_Ignore_Ind,Min_Points_Ind,Max_Points_Ind,Range_Ratio_Ind,1,0);
    ...
    if( Orders_CNT == 0 )
    {
      if( Signal == 1 ) 
      {
        ...
        int Order_Ticket= OrderSend(Symbol(),OP_BUY,Lots,Ask,Slip*Pips_Slip,0,TP,0,Magic_Number,0,0); 
        if(Order_Ticket>0&&OrderSelect(OrdersTotal()-1,SELECT_BY_POS)) 
             OrderModify(OrderTicket(),OrderOpenPrice(),Stop_Loss,0,0,0);
        else Print("OrderSend() failed: ",ErrorDescription(GetLastError()));
      ...
      {  
        ...
        Order_Ticket= OrderSend(Symbol(),OP_SELL,Lots,Bid,Slip*Pips_Slip,0,TP,0,Magic_Number,0,0);
        if(Order_Ticket>0&&OrderSelect(OrdersTotal()-1,SELECT_BY_POS)) 
             OrderModify(OrderTicket(),OrderOpenPrice(),Stop_Loss,0,0,0);
        else Print("OrderSend() failed: ",ErrorDescription(GetLastError()));
      }
        ...
        { 
          ...
          if( Stop_Loss > OrderStopLoss())
          {
            ...
            if( 2*Stop_Loss < OrderOpenPrice()+Ask)
              { if     (Orders_CNT==1) double L=Lots;
                else if(Orders_CNT==2)        L=1.5*Lots;
                else                          L=Lots;
                Order_Ticket = OrderSend(Symbol(),OP_BUY,L,Ask,Slip*Pips_Slip,0,TP,0,Magic_Number_A,0,0);
                if(Order_Ticket>0&&OrderSelect(OrdersTotal()-1,SELECT_BY_POS)) 
                     OrderModify(OrderTicket(),OrderOpenPrice(),Stop_Loss,0,0,0);
                else Print("OrderSend() failed: ",ErrorDescription(GetLastError()));
              }
      ...
      else if( OrderType() == OP_SELL) 
      {
        ...
        { iWait();
          ...
          { 
            Stop_Loss = Set_Stop_Loss(0,OP_BUY);
            iWait();
            Order_Ticket= OrderSend(Symbol(),OP_BUY,Lots,Ask,Slip*Pips_Slip,0,TP,0,Magic_Number,0,0);
            if(Order_Ticket>0&&OrderSelect(OrdersTotal()-1,SELECT_BY_POS)) 
                 OrderModify(OrderTicket(),OrderOpenPrice(),Stop_Loss,0,0,0);
            else Print("OrderSend() failed: ",ErrorDescription(GetLastError()));
          }
        ...
        { 
          ...
          if(TimeCurrent()>D'2014.3.30') return(0);
          if( Stop_Loss < OrderStopLoss() && Stop_Loss > Point)
          {
            OrderSelect(Tickets[0],SELECT_BY_TICKET);
            if( 2*Stop_Loss > OrderOpenPrice()+Bid )
              { if     (Orders_CNT==1)        L=Lots;
                else if(Orders_CNT==2)        L=1.5*Lots;
                else                          L=Lots;
                Order_Ticket = OrderSend(Symbol(),OP_SELL,L,Bid,Slip*Pips_Slip,0,TP,0,Magic_Number_A,0,0);
                if(Order_Ticket>0&&OrderSelect(OrdersTotal()-1,SELECT_BY_POS)) 
                      OrderModify(OrderTicket(),OrderOpenPrice(),Stop_Loss,0,0,0);
                else  Print("OrderSend() failed: ",ErrorDescription(GetLastError()));
              } 
    ...
    if(Orders == 1)
    {
      if(Order_Type == OP_BUY)
      {
        ...
        { 
          ...
          if( MathAbs(Low[Bars_Shift_1]-Low[iLowest(NULL,0,MODE_LOW,Reverse_Point_B,Bars_Shift_1)])<Point && 
      ...
      else if(Order_Type == OP_SELL)
      {
        for(i=Seek_Period_Ind;i>0;i--)
        {
          ...
          if( MathAbs(High[Bars_Shift_1]-High[iHighest(NULL,0,MODE_HIGH,Reverse_Point_B,Bars_Shift_1)])<Point && 
              Bars_Shift_1 >= Reverse_Point_B
             )
  ...
  { 
   ...
   if( OrderSelect(T,SELECT_BY_TICKET) && Orders_CNT==0)
   {
    ...
    if(B_S<=2 && OrderProfit()<0)
      if(OrderType()==OP_BUY && High[B_S]-Low[B_S] > 25*Pips_SL && Close[0]-Low[B_S]>(High[B_S]-Low[B_S])/2&&OrderMagicNumber()==Magic_Number_N)
       { if(!OrderSend(Symbol(),OP_BUY,Lots,Ask,3*Pips_Slip,Ask-12*Pips_SL,0,0,Magic_Number_R,0,0)) 
         Print("Error: ",ErrorDescription(GetLastError()));
       }
      else if (OrderType()==OP_SELL && High[B_S]-Low[B_S] > 25*Pips_SL && High[B_S]-Close[0]>High[B_S]-Low[B_S]/2&&OrderMagicNumber()==Magic_Number_N)
       { if(!OrderSend(Symbol(),OP_SELL,Lots,Bid,3*Pips_Slip,Bid+12*Pips_SL,0,0,Magic_Number_R,0,0))
         Print("Error: ",ErrorDescription(GetLastError()));
       }
   }
   return;
