//+------------------------------------------------------------------+
//|                                                                  |
//|            more details to depict Move_Stop_Loss function        |
//|                                                                  |
//+------------------------------------------------------------------+
...
{                                 // bigger than small stop level, and when order profit is more than 20 Point_Adapted, the stop level  
  if(OrdersTotal()==0) return;    // should be changed to big stop level from small small stop level.
                                  // Notes: when the max profit is less than 40 Point_Adapted, the real stop loss is small; only when the 
  int Move_Stop;                  //        max profit is more than 40 Point_Adapted, the real stop loss is big.
  for(int i=OrdersTotal();i>0;i--)
  {
    Move_Stop=Move_Stop_Small;
    OrderSelect(i-1,SELECT_BY_POS);
    if(OrderProfit()>20*Point_Adapted) Move_Stop=Move_Stop_Big;  //delete "||MathAbs(OrderStopLoss()-Close[0])>Move_Stop_Small"
    if(MathAbs(Close[0]-OrderStopLoss())<=Move_Stop*Point_Adapted) continue;
    if(OrderType()==OP_BUY) {iWait();OrderModify(OrderTicket(),OrderOpenPrice(),Close[0]-Move_Stop*Point_Adapted,0,0,0);} 
    else  {iWait();OrderModify(OrderTicket(),OrderOpenPrice(),Close[0]+Move_Stop*Point_Adapted,0,0,0);}
  }
  return;
}
