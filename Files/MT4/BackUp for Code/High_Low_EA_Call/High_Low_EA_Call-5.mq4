...
*/
  ...
  double Trade_Sig=iCustom(NULL,0,"High_Low_Ind",15,10,9,2,0);
  int Shift_Bar=iCustom(NULL,0,"High_Low_Ind",15,10,9,2,1);
...
{                                 // bigger than small stop level, and when order profit is more than 20 points, the stop level should 
  ...
  for(int i=OrdersTotal();i>0;i--)
  {
    if(OrderSelect(i-1,SELECT_BY_POS))
    {
      ...
      if(Profit_Point>Move_Stop_Small&&Profit_Point<=500) Move_Stop=Move_Stop_Big;
      if(Profit_Point>500)Move_Stop=Move_Stop_Small/2;
