//+------------------------------------------------------------------+
//|        Not modified well but all is ok                           |
//|            no error about procedure                              |
//|                                                                  |
//+------------------------------------------------------------------+
...
extern int Seek_Period=15,Mini_Swing=10,Trade_Range=6; //Mini_Swing_Left and Mini_Swing_Right;
...
{  
   ...
   for(int i=Limit;i>=0;i--)
   {
     ...
     while(P_CNT>=10)
     {
       int x=iHighest(NULL,0,MODE_HIGH,P_CNT,i); if(x-i<10) break;
       ...
       if(High_Refer_0==High[x]&&High_Refer_0-Low_Refer_0>=Mini_Swing*Point_Adapted) //peak confirm
       {
         ...
         if(i==0&&Low[0]!=Low_Refer_0&&H_Current_0!=High[0]&&High_Refer_0-Close[0]<7*Point_Adapted&&
         H_Current_0>=High_Refer_0-Trade_Range*Point_Adapted&&H_Current_0<=High_Refer_0+Trade_Range*Point_Adapted/2 //touch trading ranges
            &&Close[0]<L_Current_0   //triger trading
           ){Call[0]=-1;break;}
       }
       P_CNT--;        
     }
     ...
     while(P_CNT>=10)
     {
       int y=iLowest(NULL,0,MODE_LOW,P_CNT,i); if(y-i<10) break;
       ...
       if(Low_Refer_1==Low[y]&&High_Refer_1-Low_Refer_1>=Mini_Swing*Point_Adapted) //lawn confirm
       {
         ...
         if(i==0&&High[0]!=High_Refer_1&&L_Current_1!=Low[0]&&Close[0]-Low_Refer_1<7*Point_Adapted&&
         L_Current_1<=Low_Refer_1+Trade_Range*Point_Adapted&&L_Current_1>=Low_Refer_1-Trade_Range*Point_Adapted/2  //touch trading ranges
            &&Close[0]>H_Current_1    //triger trading
           ){Call[0]=1;break;}
       }
       P_CNT--;    
     }
