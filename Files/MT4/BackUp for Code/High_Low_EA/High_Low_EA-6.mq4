//+------------------------------------------------------------------+
//|                                                                  |
//|         Trade_Range*Point_Adapted/2 changed from                 |
//|         Trade_Range/2*Point_Adapted                              |
//+------------------------------------------------------------------+
...
{  
  ...
  while(P_CNT>=6)
  {
    G_i=iHighest(NULL,0,MODE_HIGH,P_CNT,0);if(G_i<5) break;
    double High_Refer_P=High[iHighest(NULL,0,MODE_HIGH,10,G_i-5)],Low_Refer_P=Low[iLowest(NULL,0,MODE_LOW,G_i,0)],
    HL_Current_P=High[iHighest(NULL,0,MODE_HIGH,3,0)]; 
    if(High_Refer_P==High[G_i]&&High[G_i]-Low_Refer_P>=Mini_Swing*Point_Adapted) // peak confirm
    { 
  ...
  while(P_CNT>=6)
  {
    D_i=iLowest(NULL,0,MODE_LOW,P_CNT,0); if(D_i<5) break;
    ...
    if(Low_Refer_P==Low[D_i]&&High_Refer_P-Low[D_i]>=Mini_Swing*Point_Adapted) // bottom confirm
    { 
      if(Show_Con==0){Show_Refer_Point("Low",D_i,0);Show_Con=1;}
      if(HL_Current_P!=Low[0]&&HL_Current_P<=Low[D_i]+Trade_Range*Point_Adapted/2&&HL_Current_P>=Low[D_i]-Trade_Range*Point_Adapted&& //touch trading ranges
