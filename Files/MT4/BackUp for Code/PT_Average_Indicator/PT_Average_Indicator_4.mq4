...
int start()
{
  ...
  for(int i = Limit; i >= 0; i--)
  {
    ...
    PT_Buffer[i] = iMA(NULL, 0, P_Point/Time_Frame, 0, MODE_SMA, PRICE_CLOSE, i);
    int P_Point_1=(High[iHighest(NULL, 0, MODE_HIGH, 12*Seek_Period, i)]-Low[iLowest(NULL, 0, MODE_HIGH, 12*Seek_Period, i)])/Point;
    PT_Buffer_1[i] = iMA(NULL, 0, P_Point_1/Time_Frame, 0, MODE_SMA, PRICE_CLOSE, i);
  }
...
*/
 ...
 if(Close[0]-PT_Buffer[0]>=28*Point&&Low[iLowest(NULL,0,MODE_LOW,5,0)]>PT_Buffer[0]-50*Point)//buy
 { 
   ...
   if(Check==true&&Uniformity(CNT)==1) { Signal[0]=0.0; return(0);} // ||Uni==0&&Close[0]-Low[iLowest(NULL,0,MODE_LOW,Seek_Period,0)]<150*Point
 }
  if(PT_Buffer[0]-Close[0]>=28*Point&&High[iHighest(NULL,0,MODE_HIGH,5,0)]<PT_Buffer[0]+50*Point)//sell
 { 
   ...
   if(Check==true&&Uniformity(CNT)==0) { Signal[0]=1.0; return(0);} //||Uni==1&&High[iHighest(NULL,0,MODE_HIGH,Seek_Period,0)]-Close[0]<150*Point
 }
