//+------------------------------------------------------------------+
//|    just for backup; catch all the opening that is strong reverse |
//|    no needs big Stop_Loss and ignore some rare bars whosing      |
//|    Coming means a weak reverse                                   |
//+------------------------------------------------------------------+

int Stop_Loss=50; //Stop_Loss doesn't need too big because in strong trend,reverse behavior will be not too strong.
...
{                                 // bigger than small stop level, and when order profit is more than 20 points, the stop level should 
  ...
  for(int i=OrdersTotal();i>0;i--)
  {
    if(OrderSelect(i-1,SELECT_BY_POS))
    {
      ...
      int Move_Stop=80;
      if(Profit_Point>0&&Profit_Point<=500) Move_Stop=200;
      if(Profit_Point>500)Move_Stop=50;
